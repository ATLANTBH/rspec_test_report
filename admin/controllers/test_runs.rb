...
Admin.controllers :test_runs do
  ...
  get :index do
    query = "select min(id) as id, max(duration) as total_duration, sum(example_count) as all_examples, sum(failure_count) as all_failures, sum(pending_count) as all_pending, max(computer_name) as computer_name, min(created_at) as created_at, max(updated_at) as updated_at, max(test_suites_id) as test_suites_id, build from test_runs where test_suites_id = ? group by build order by updated_at desc", params[:test_suite_id]
